...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function filtrarProspectos(filtro, valor) {
    ...
    if (filtro === "fecha_create") {
      // Añadir un día al máximo para incluir todo el día 16
      const maxDate = new Date(valor.max);
      maxDate.setDate(maxDate.getDate() + 1);

      query = query
        .where("fecha_create", ">=", valor.min.getTime()) // Cambiado de ">" a ">="
     ...
     if (!asesoresSnapshot.empty) {
         // Obtener los IDs de los asesores encontrados
         const asesorIds = asesoresSnapshot.docs.map((doc) => doc.id);

         // Filtrar los prospectos que coincidan con esos IDs de asesor
         query = query
  ...
  async function cargarMasProspectos() {
    if (!lastVisible || !currentQuery) return;
